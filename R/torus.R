...
torus <- function(x1,
  ...
  cD1 <- cDiff(q[1])
  cD2 <- cDiff(q[2])
  P1 <- crossprod(cD1) %x% diag(q[2])
  P2 <- diag(q[1]) %x% crossprod(cD2)

  # add additional constraints, see Boer (2023).
  cB1_star <- cBsplines(knots[[1]], x = c(0.00, 0.25))
  cB2_star <- cBsplines(knots[[2]], x = c(0.00, 0.25))
  cB_star <- cB1_star %x% cB2_star

  P1 <- P1 + crossprod(cB_star)
  P2 <- P2 + crossprod(cB_star)

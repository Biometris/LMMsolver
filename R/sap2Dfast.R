...
#'
#' @export
sap2Dfast <- function(y, x1, x2, knots, trace=TRUE, thr=1.0e-8, scaleX=FALSE)
{
  nseg <- knots
  n <- length(y)
  s <- proc.time()[3]
  ...
  CCt <- as.spam(C %*% t(C))
  lGinv <- list()
  lGinv[[1]] <- DtD1 %x% diag.spam(q2) + CCt
  lGinv[[2]] <- diag.spam(q1) %x% DtD2 + CCt

  names(lGinv) <- c('x1','x2')

  obj = sparseMixedModels(y, X, B12, lGinv, lRinv,
                          maxit=200, tolerance=thr, display=FALSE, trace=trace)
...
#'
#' @export
predict.sap2Dfast <- function(object, grid)
{

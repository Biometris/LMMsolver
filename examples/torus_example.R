...
sim_fun <- function(x) {exp(sin(2*pi*x[1]))+
                             exp(cos(2*pi*x[2])) }
...
ggplot() +
  ...
  scale_fill_gradientn(colors = topo.colors(100)) +
  coord_fixed() + ggtitle("Training: simulated plus noise") + JOPS_theme()
...
grid <- expand.grid(x1=seq(0,1,length=100),x2=seq(0,1,length=100))
x1 <- grid$x1
x2 <- grid$x2

...
grid1 <- seq(-5, 5, by = 0.05)
grid2 <- seq(-5, 5, by = 0.05)
theta1 <- 10^grid1
theta2 <- 10^grid2
...
lRinv <- object$lRinv
y <- object$y
df_logL <- LMMsolver:::logLikelihood(y,X,Z,lGinv,lRinv, theta = theta)
toc()
...
theta_max
LMMsolver:::logLikelihood(y,X,Z,lGinv,lRinv, theta = theta_max)

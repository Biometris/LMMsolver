...
# Smooth trend with plots in a single line.
...
In the following mixed model we include rep and gen as fixed effect, and `spl1D` to model a one dimensional P-spline [@Eilers1996] with 20 segments, and using the default choice of cubical B-splines and second order differences:
...
The smooth trend along the field can be obtained as follows:
...
The two-dimensional P-spline can be defined with the `spl2D()` functions, and with latitude and longitude as covariates. The number of segments chosen here is equal to as used in @Rodriguez-Alvarez2015.

...
## Smooth spatial trend in one direction

As a first example we will use an oats trial from the `agridat` package. There were 24 varieties in 3 replicates, each consisting of 6 incomplete blocks of 4 plots. The plots were laid out in a single line. For further details of the analysis of this data set using P-splines see @Boer2020.
...
In the following mixed model we include rep and gen as fixed effect, and we use `spl1D` to model a one dimensional P-spline [@Eilers1996] with 200 segments, and the default choice of cubical B-splines and second order differences:

```{r Pspline1D}
## Fit mixed model with fixed and spline part.
obj1 <- LMMsolve(fixed = yield ~ rep + gen,
                 spline = ~spl1D(x = plot, nseg = 200),
                 data = john.alpha)
```
A high number of segments can be used for splines in one dimension, as the corresponding mixed model equations are sparse. 
...
summary(obj2)
```
where `splF` is the fixed part of the spline, the linear trend. 
...
The two-dimensional P-spline can be defined with the `spl2D()` functions, and with longitude and latitude as covariates. The number of segments chosen here is equal to as used in @Rodriguez-Alvarez2015.
...
```{r groupOPTION}
## Fit alternative model - include QTL.
lGrp <- list(QTL = 3:5)

...
For two-dimensional mixed P-splines as defined in @boer2023 we will two examples. The first one for US precipitation data. The second one modelling Sea Surface Temperature (SST) as described in @cressie2022.
...
obj <- LMMsolve(sst~1,spline=~spl2D(lon,lat,nseg=nseg,
...
pred_grid <- obtainSmoothTrend(obj, newdata=grid_over_D, includeIntercept = TRUE)
...
# make a plot
plotDat <- pred_grid
plotDat <- plotDat[plotDat$se<5, ]

## Plot prediction
plot_predictions <- ggplot(plotDat) +

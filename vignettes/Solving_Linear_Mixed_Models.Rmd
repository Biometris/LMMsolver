...
summary(obj2)
```
The effective dimension is a measure for complexity. In the table above the first three terms are fixed effect and penalized, and therefore the effective dimension is equal to the number of parameters in the model. The term `s(x)` is a random effect, with effective dimension is \code{14.2}, indicating that there is rather strong non-linear smooth trend, corresponding to what we see in the Figure.  

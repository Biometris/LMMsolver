...
The `LMMsolve` package can fit models where the matrices $G^{-1}$ and $R^{-1}$ are a linear combination of precision matrices $Q_{G,i}$ and $Q_{R,i}$: $$
  G^{-1} = \sum_{i} \psi_i Q_{G,i} \;, \quad   R^{-1} = \sum_{i} \psi_i Q_{R,i} 
$$ where the precision parameters $\psi_i$ and $\phi_i$ are estimated using REML. For most standard mixed models $1/{\psi_i}$ are the variance components and $1/{\phi_i}$ the residual variance(s), but we use a formulation in terms of precision parameters to allow for non-standard mixed models using tensor product splines [\@Rodriguez-Alvarez2014].
...
In the following mixed model we include rep and gen as fixed effect, and `spl1D` to model a one dimensional P-spline [@Eilers1996] with 20 segments, using the default choice of cubical B-splines and second order differences:
...
ggplot(data = plotDat, aes(x = plot, y = ypred)) +
  ...
  labs(title = "Smooth Trend Oats data", x = "plotnr", y = "yield") +
...
see the P-spline(1) model in Table 1 in @Boer2020.
...
The effective dimension is a measure for complexity. In the table above the first three terms are fixed effects and not penalized, and therefore the effective dimension is equal to the number of parameters in the model. The term `s(plot)` is a random effect, with effective dimension \code{14.2}, indicating that there is rather strong non-linear smooth trend in the field, corresponding to what we see in the Figure.
...
where we took a high number of segments (`nseg=200`), as the computations are very fast because of the sparse structure of the mixed model equations. The effective dimension are:
...
For two-dimensional mixed P-splines we use the model defined in @Rodriguez-Alvarez2014. The use of sparse matrix algebra makes the computations even faster as reported in @Rodriguez-Alvarez2014. As an example we use the `USprecip` data set in the `spam` package [@Furrer2010], analysed in @Rodriguez-Alvarez2014.
...
The two-dimensional P-spline can be defined with the `spl2D()` functions, and with latitude and longitude as covariates. The number of segments chosen here is equal to as used in @Rodriguez-Alvarez2014.
